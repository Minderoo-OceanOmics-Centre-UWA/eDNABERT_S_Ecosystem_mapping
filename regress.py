...
def prepare_data(embeddings_df: pd.DataFrame, coords_df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    merged['base_site'] = merged['site_id'].astype(str).str.replace(r'_\d+$', '', regex=True)

    print(f"[Data] Merged {len(merged)} rows (replicates) with both embeddings and coordinates")
    ...
    print(f"[Data] Features: {X.shape[1]} dimensions, Targets: {len(merged)} replicate rows across {n_sites} base sites")
...
def main():
    parser = argparse.ArgumentParser(description="Predict lat/lon from site embeddings with grouped CV by base_site")
    ...
    print(f"[Split] Training replicates: {len(X_train)} across {pd.unique(groups_train).size} base sites")
    print(f"[Split] Testing replicates:  {len(X_test)} across {pd.unique(groups_test).size} base sites")
    ...
    if args.model == "ridge":
        ...
        if args.optimize_hyperparams:
            ...
            model_name = f"Ridge Regression (alpha={model.best_estimator_.estimator.alpha})"
        else:
